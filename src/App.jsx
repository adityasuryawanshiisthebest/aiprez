...
function validateCreatePresentationPrompt(input) {
  ...
  const numericSlideMatch = normalized.match(/\b(\d{1,2})\s*(?:-| )?\s*(?:slide|slides|page|pages)\b/);
  const requestedSlideCount = numericSlideMatch ? Number(numericSlideMatch[1]) : null;
  ...
  const hasLayout = /\b(?:landscape|portrait|wide|widescreen|horizontal|vertical|16:9|4:3|layout|mode)\b/.test(normalized);
  if (!hasLayout) {
    return "Include the layout design, like landscape, portrait, or widescreen.";
  }

  const hasTheme = /\b(?:theme|style|palette|color|colors|colour|colours|vibe|design)\b/.test(normalized);
  if (!hasTheme) {
...
function normalizeGeneratedDeck(deck, output = "", prompt = "") {
  if (!deck || !Array.isArray(deck.slides)) return null;
  const tones = ["cover", "split", "timeline", "map", "comparison", "data", "quote", "gallery"];
  return {
    ...
    slides: deck.slides.map((slide, index) => ({
      ...
      title: cleanMarkdownText(slide.title) || `Slide ${index + 1}`,
      ...
      previewBullets: Array.isArray(slide.previewBullets)
        ? slide.previewBullets.map(cleanMarkdownText).filter(Boolean).slice(0, 5)
        : [],
      ...
      palette: Array.isArray(slide.palette) && slide.palette.length
        ...
        : Array.isArray(deck.themePalette)
          ...
          : [],
      thumb: cleanMarkdownText(slide.title) || `Slide ${index + 1}`,
...
function parseGeneratedPresentation(output, prompt = "", deckPayload = null) {
  const normalizedDeck = normalizeGeneratedDeck(deckPayload, output, prompt);
  ...
  const title =
    cleanMarkdownText(titleMatch?.[1])
      .split(/\n/)
      .find(Boolean) ||
    cleanMarkdownText(prompt).split(/[.!?]/)[0] ||
  ...
  const slides = slideMatches.map((match, index) => {
    const body = match[3] || "";
    const bullets = [...body.matchAll(/^\s*-\s+(.+)$/gm)].map((item) => cleanMarkdownText(item[1]));
    const speakerNotes = cleanMarkdownText(
      body.match(/\*\*Speaker notes suggestion:\*\*\s*([\s\S]*?)(?=\n---|\n\s*###|\n\s*##|$)/i)?.[1] || ""
    );
...
function SlideThumbnail({ number, title, active, tone, image, onClick }) {
  return (
...
function GeneratedSlideVisual({ slide }) {
  if (slide?.imageDataUrl) {
    return (
      <div className="generated-slide-image" aria-hidden="true">
        <img src={slide.imageDataUrl} alt="" />
      </div>
    );
  }

  return (
    <div className={`generated-slide-visual ${slide?.tone || "cover"}`} aria-hidden="true">
...
function PresentationPreview({ mode = "humanizer", generatedDeck }) {
  const [activeSlide, setActiveSlide] = React.useState(0);
  const slides = generatedDeck?.slides || [];
  const slide = slides[activeSlide] || slides[0];
  const visibleBullets = slide?.previewBullets?.length ? slide.previewBullets : slide?.bullets || [];
  const visibleStart = slides.length > 4 ? Math.min(Math.max(activeSlide - 1, 0), slides.length - 4) : 0;
  const visibleSlides = slides.slice(visibleStart, visibleStart + 4);
  ...
  return (
    <section className="preview-panel glass generated-preview-panel">
      <h3>Presentation Preview</h3>
      <div className="upload-row generated-upload-row">
        <span className="ppt-icon">AI</span>
        <strong>{generatedDeck.title}</strong>
        <span className="uploaded-pill">
          Generated <Icon icon={CheckCircle} size={15} strokeWidth={1.8} />
        </span>
      </div>
      <div
        className={`main-slide generated-main-slide ${slide.tone} ${slide.imageDataUrl ? "has-image" : ""}`}
        style={{
          "--slide-accent-a": slide.palette?.[0] || "#ff4f75",
          "--slide-accent-b": slide.palette?.[1] || "#ffbd3a",
          "--slide-accent-c": slide.palette?.[2] || "#2c8cff",
        }}
      >
        <GeneratedSlideVisual slide={slide} />
        <div className="slide-copy generated-slide-copy">
          <small>Slide {activeSlide + 1}</small>
          <h4>{slide.title}</h4>
          <span></span>
          <p>{slide.subtitle}</p>
        </div>
        {visibleBullets.length > 0 && (
          <div className="generated-detail-grid">
            {visibleBullets.slice(0, 5).map((bullet, index) => (
        ...
        {slide.speakerNotes && (
          <div className="generated-notes-strip">
            <strong>Speaker Insight</strong>
            <span>{slide.speakerNotes}</span>
          </div>
        )}
        {slide.visualDirection && <em className="generated-visual-note">{slide.visualDirection}</em>}
      </div>
      <div className="slide-strip">
        <button className="prev-slide" type="button" aria-label="Previous slide" onClick={() => setActiveSlide((index) => (index - 1 + slides.length) % slides.length)}>
        ...
        <button className="next-slide" type="button" aria-label="Next slide" onClick={() => setActiveSlide((index) => (index + 1) % slides.length)}>
          <Icon icon={ChevronDown} size={25} strokeWidth={1.7} />
        </button>
      </div>
  ...
  return (
    <aside className="ai-spec-panel glass">
      ...
      <div className="humanizer-chat">
        <HumanizerMessage ai>
          {isCreate
            ? "Hi Ava! I can create your presentation once you include: 1. topic, 2. number of slides (1-15), 3. layout design like landscape or portrait, and 4. visual theme like black and blue or white and green."
      ...
      <InstructionComposer
        ...
        onComplete={
          isCreate
            ? (result, prompt) => {
                onPresentationGenerated?.(parseGeneratedPresentation(result.output, prompt, result.deck));
                return {
...
function HumanizerWorkspace({ onBack, mode = "humanizer", title, subtitle, settingsLabel }) {
  const [generatedDeck, setGeneratedDeck] = React.useState(null);
  ...
  return (
    <main className="humanizer-workspace">
      ...
      <div className="humanizer-content">
        <PresentationPreview mode={mode} generatedDeck={mode === "create" ? generatedDeck : null} />
        <AISpecifications mode={mode} onPresentationGenerated={setGeneratedDeck} />
      </div>
      <BottomUtilityBar />
    </main>
  );
